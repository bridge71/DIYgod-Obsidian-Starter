...
let total = 0;
for (let page of window.pages) {
 if (page['Class and Homework']) {
     count++;
     const [hour, min] = page['Class and Homework'].split(':')
     total += (+hour + +min / 60 )
 }
}
...
dv.el('div', score + '%', {
    cls: score > 80 ? 'score-class3' : score > 50 ? 'score-class2' : 'score-class1'
});
```
...
let count = 0;
for (let page of window.pages) {
 if (page['Reading'] || page['REciting'] || page['Blogging'] ) {
     count++;
 }
}
const score = (count / window.pages.length * 100).toFixed(2);
